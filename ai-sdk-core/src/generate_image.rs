...
///
/// **Deprecated:** Use the `GenerateImage` builder instead.
///
/// # Arguments
///
/// * `model` - The image model to use.
/// * `prompt` - The prompt that should be used to generate the image.
/// * `n` - Number of images to generate. Default: 1.
/// * `size` - Size of the images to generate in format `{width}x{height}` (e.g., "1024x1024").
/// * `aspect_ratio` - Aspect ratio of the images to generate in format `{width}:{height}` (e.g., "16:9").
...
async fn generate_image(
    ...
    abort_signal: Option<CancellationToken>,
    headers: Option<SharedHeaders>,
) -> Result<GenerateImageResult, AISDKError> {
    // Check specification version
    ...
    // Parse size and aspect ratio if provided
    let parsed_size = if let Some(ref size_str) = size {
        Some(
            ImageSize::from_str(size_str)
                .map_err(|e| AISDKError::invalid_argument("size", size_str, e))?,
        )
    ...
    let parsed_aspect_ratio = if let Some(ref aspect_ratio_str) = aspect_ratio {
        Some(
            AspectRatio::from_str(aspect_ratio_str)
                .map_err(|e| AISDKError::invalid_argument("aspect_ratio", aspect_ratio_str, e))?,
        )
    } else {
        None
    };
    ...
    // Get max images per call from model or use provided value
    let max_images_per_call_with_default = if let Some(max) = max_images_per_call {
        max
    } else {
        model
            .max_images_per_call(model.model_id())
            .await
            .unwrap_or(1) as u32
    };
