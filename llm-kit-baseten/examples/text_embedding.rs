...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("📊 Baseten Text Embedding Example (Provider-Only)\n");
    ...
    let model_id = std::env::var("BASETEN_EMBEDDING_MODEL_ID").map_err(|_| {
        "BASETEN_EMBEDDING_MODEL_ID environment variable not set. \
         Please set it with your embedding model ID from Baseten."
    })?;

    println!("✓ API key loaded from environment");
    println!("✓ Model ID loaded from environment");
    ...
    // Create Baseten provider with custom model URL for embeddings
    ...
    let texts = vec![
        ...
        "Baseten offers powerful model deployment infrastructure.".to_string(),
    ];
    ...
    println!("\n💡 Note: Baseten embeddings require a custom model URL.");
    println!("   Set BASETEN_EMBEDDING_MODEL_ID to your deployment's model ID.");
