use super::{MessageMetadata, MessageRole};
use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};

/// An assistant message in a conversation session.
///
/// Assistant messages represent responses from the AI model and can contain
/// multiple parts such as text, reasoning, tool calls, and tool results.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct AssistantMessage {
    /// Unique message identifier
    pub id: String,

    /// Session this message belongs to
    pub session_id: String,

    /// Role (always Assistant)
    pub role: MessageRole,

    /// IDs of parts that comprise this message
    pub part_ids: Vec<String>,

    /// Message metadata (includes model_id, usage, etc.)
    pub metadata: MessageMetadata,

    /// When this message was created
    pub created_at: DateTime<Utc>,
}

impl AssistantMessage {
    /// Create a new assistant message.
    ///
    /// # Arguments
    ///
    /// * `id` - Unique identifier for the message
    /// * `session_id` - ID of the session this message belongs to
    /// * `part_ids` - IDs of the message parts
    pub fn new(id: String, session_id: String, part_ids: Vec<String>) -> Self {
        Self {
            id,
            session_id,
            role: MessageRole::Assistant,
            part_ids,
            metadata: MessageMetadata::default(),
            created_at: Utc::now(),
        }
    }

    /// Set the metadata for this message (builder pattern).
    pub fn with_metadata(mut self, metadata: MessageMetadata) -> Self {
        self.metadata = metadata;
        self
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::message::UsageStats;

    #[test]
    fn test_assistant_message_creation() {
        let message = AssistantMessage::new(
            "msg-123".to_string(),
            "session-456".to_string(),
            vec!["part-1".to_string(), "part-2".to_string()],
        );

        assert_eq!(message.id, "msg-123");
        assert_eq!(message.session_id, "session-456");
        assert_eq!(message.role, MessageRole::Assistant);
        assert_eq!(message.part_ids.len(), 2);
    }

    #[test]
    fn test_assistant_message_with_metadata() {
        let metadata = MessageMetadata {
            model_id: Some("gpt-4".to_string()),
            provider: Some("openai".to_string()),
            usage: Some(UsageStats::new(100, 50)),
            finish_reason: Some("stop".to_string()),
            custom: None,
        };

        let message = AssistantMessage::new(
            "msg-123".to_string(),
            "session-456".to_string(),
            vec!["part-1".to_string()],
        )
        .with_metadata(metadata.clone());

        assert_eq!(message.metadata, metadata);
    }

    #[test]
    fn test_assistant_message_serialization() {
        let message = AssistantMessage::new(
            "msg-123".to_string(),
            "session-456".to_string(),
            vec!["part-1".to_string()],
        );

        let json = serde_json::to_string(&message).unwrap();
        let deserialized: AssistantMessage = serde_json::from_str(&json).unwrap();
        assert_eq!(message.id, deserialized.id);
        assert_eq!(message.session_id, deserialized.session_id);
        assert_eq!(message.role, deserialized.role);
    }
}
