/// Image generation example using Azure OpenAI provider with only llm-kit-provider.
///
...
/// cargo run --example image_generation -p llm-kit-azure
/// ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🎨 Azure OpenAI Image Generation Example (Provider-Only)\n");

    // Get credentials from environment
    let api_key = std::env::var("AZURE_API_KEY").map_err(|_| {
        "AZURE_API_KEY environment variable not set. Please set it with your API key."
    })?;
    let resource_name = std::env::var("AZURE_RESOURCE_NAME").map_err(|_| {
        "AZURE_RESOURCE_NAME environment variable not set. Please set it with your resource name."
    })?;

    println!("✓ API key loaded from environment");
    println!("✓ Resource name loaded from environment");

    // Create Azure OpenAI provider using client builder
    let provider = AzureClient::new()
        .resource_name(resource_name)
        .api_key(api_key)
        .build();

    // Create an image model (use your DALL-E deployment name)
